...
+ (NSString *)stringForPrivacyType:(ECPrivacyType)privacyType;
{
    if (privacyType == ECPrivacyType_LocationServices) {
        return NSLocalizedString(@"AuthorizationLocationServices", nil);
    }else if (privacyType == ECPrivacyType_Contacts) {
        return NSLocalizedString(@"AuthorizationContacts", nil);
    }else if (privacyType == ECPrivacyType_Calendars) {
        return NSLocalizedString(@"AuthorizationCalendars", nil);
    }else if (privacyType == ECPrivacyType_Reminders) {
        return NSLocalizedString(@"AuthorizationReminders", nil);
    }else if (privacyType == ECPrivacyType_Photos) {
        return NSLocalizedString(@"AuthorizationPhotos", nil);
    }else if (privacyType == ECPrivacyType_BluetoothSharing) {
        return NSLocalizedString(@"AuthorizationBluetoothSharing", nil);
    }else if (privacyType == ECPrivacyType_Microphone) {
        return NSLocalizedString(@"AuthorizationMicrophone", nil);
    }else if (privacyType == ECPrivacyType_SpeechRecognition) {
        return NSLocalizedString(@"AuthorizationSpeechRecognition", nil);
    }else if (privacyType == ECPrivacyType_Camera) {
        return NSLocalizedString(@"AuthorizationCamera", nil);
    }else if (privacyType == ECPrivacyType_Health) {
        return NSLocalizedString(@"AuthorizationHealth", nil);
    }else if (privacyType == ECPrivacyType_HomeKit) {
        return NSLocalizedString(@"AuthorizationHome", nil);
    }else if (privacyType == ECPrivacyType_MediaAndAppleMusic) {
        return NSLocalizedString(@"AuthorizationMedia And AppleMusic", nil);
    }else if (privacyType == ECPrivacyType_MotionAndFitness) {
        return NSLocalizedString(@"AuthorizationMotion And Fitness", nil);
    }else{
        return @"";
    }
}
...
#pragma mark -- 扩展方法
+ (void)goSetting:(ECPrivacyType)type presentVC:(UIViewController *)vc
{
    NSString *title = [ECAuthorizationTools stringForPrivacyType:type];
    NSString *message = [NSString stringWithFormat:NSLocalizedString(@"AuthorizationNotAllowedMessage", nil), title];
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:title message:message preferredStyle:UIAlertControllerStyleAlert];
    [alert addAction:[UIAlertAction actionWithTitle:NSLocalizedString(@"AuthorizationSettingButtonTitle", nil) style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        NSURL *url = [NSURL URLWithString:UIApplicationOpenSettingsURLString];
        [[UIApplication sharedApplication] openURL:url];
    }]];
    [alert addAction:[UIAlertAction actionWithTitle:NSLocalizedString(@"AuthorizationOKButtonTitle", nil) style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
...
+ (void)deviceNotSupport:(ECPrivacyType)type presentVC:(UIViewController *)vc
{
    NSString *title = [ECAuthorizationTools stringForPrivacyType:type];
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:title message:@"设备不支持" preferredStyle:UIAlertControllerStyleAlert];
